...
> 허나 인증 필터는 요청으로 들어온 인증 정보를 `Authentication` 객체로 변환하는 것이 주 임무임으로 예외를 throw 하는 대신에 null 을 반환한다.
> 
> Provider 를 호출하는 ProviderManager 에서 처리해주는 예외는 `AuthenticationException` 를 상속한 예외만 처리해주니
...
>                                            // 다른 URI 의 API 호출 시에는 accessTokenFilterChain 에서 생성한 SecurityFilterChain 이 적용된다. 
>             .authorizeRequests()
>             ...;
> 
>         return http.build();
